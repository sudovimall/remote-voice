...
- 新增文档必须在标题下方保留创建时间、更新时间和更新概述，格式如下：
  - `创建时间：YYYY-MM-DD HH:MM:SS TZ`
  - `更新时间：YYYY-MM-DD HH:MM:SS TZ`
  - `更新概述：一句话说明本次新增或更新内容。`
- 修改已有文档时，必须同步更新时间，并用简短中文概述说明本次更新内容。
- 阶段进度文档和会话交接文档必须写明完成进度、修改文件、验证结果、未完成事项和下一阶段目标。
- 只做文档阶段时不运行代码测试，但必须检查 `git status --short` 并确认提交范围只包含本阶段相关文档。
