...
读取时做边界裁剪。播放音量裁剪到 `0..1`，麦克风输入增益裁剪到 `0..2`。非法、缺失或不可解析值回退到 `1`。
...
麦克风 `200%` 增益可能造成失真，这是用户可控的本地设置。远端成员播放音量只保留浏览器原生可靠的 `0%` 到 `100%` 范围；后续如果需要放大远端成员声音，需要额外引入远端 Web Audio gain 链路。
